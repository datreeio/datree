...
func (s resourceMinimumSchema) Validate(ctx jsonschema.ValidationContext, dataValue interface{}) error {
	...
	if !validStr {
		return ctx.Error(CustomKeyValidationErrorKeyPath, "%s must be a string", dataValueStr)
	}

	dataValueParsedQty, err := resource.ParseQuantity(dataValueStr)
	if err != nil {
		return ctx.Error(CustomKeyValidationErrorKeyPath, "failed parsing data value %v", dataValue)
	}

	schemaResourceMinParsedQty, err := resource.ParseQuantity(schemaResourceMinStr)
	if err != nil {
		return ctx.Error(CustomKeyValidationErrorKeyPath, "failed parsing schema value %v", schemaResourceMinStr)
	}
	...
	if err != nil {
		return ctx.Error(CustomKeyValidationErrorKeyPath, "failed float parsing value %v", dataValueFloat)
	}

	schemaMinValueFloat, err := strconv.ParseFloat(schemaResourceMinParsedQtyDecimal, 64)
	if err != nil {
		return ctx.Error(CustomKeyValidationErrorKeyPath, "failed float parsing value %v", schemaMinValueFloat)
	}
...
func (s resourceMaximumSchema) Validate(ctx jsonschema.ValidationContext, dataValue interface{}) error {
	...
	if err != nil {
		return ctx.Error(CustomKeyValidationErrorKeyPath, "failed parsing schema value %v", schemaResourceMaxStr)
	}
	...
	if err != nil {
		return ctx.Error(CustomKeyValidationErrorKeyPath, "failed float parsing value %v", schemaMaxValueFloat)
	}
